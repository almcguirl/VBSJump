...
VBCLiquidCrystal_I2C lcd(0x27, 16, 2);
...
void moveOneStep() {
  ...
  digitalWrite( stepA, (out & 0x01) ? HIGH : LOW );
  digitalWrite( stepB, (out & 0x02) ? HIGH : LOW );
  digitalWrite( stepC, (out & 0x04) ? HIGH : LOW );
  digitalWrite( stepD, (out & 0x08) ? HIGH : LOW );
}

void setup(){
  // Set pullup the inptus and outputs of the Arduino
...
void loop(){
  ...
  if (!playing) {
    ...
    delay(250);
    blink = !blink;
    if (buttonPushed ||  analogRead(PIN_JOYSTICK) > 600) {
